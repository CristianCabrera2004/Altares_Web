...
export class PdfService {
  ...
  generarPdfReciboGlobal(
    ...
    if (itemsFactura.length > 0) {
      doc.setFontSize(11);
      doc.setFont('helvetica', 'bold');
      doc.text(`Detalle de Productos`, 10, startY);
      startY += 5;
    ...
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text('Resumen de Ventas del Día', 10, startY);
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Efectivo: ${this.currency(totalEfectivo)}`, 15, startY + 7);
    doc.text(`Transferencia: ${this.currency(totalTransferencia)}`, 15, startY + 12);
    
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text(`TOTAL GENERAL: ${this.currency(totalEfectivo + totalTransferencia)}`, 10, startY + 22);
